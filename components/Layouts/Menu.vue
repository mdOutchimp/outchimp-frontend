<template>
  <div>
    <nav class="menu fixed-top">
      <div class="menu__container">
        <div class="brand flex-grow-1" v-if="appSettings">
          <NuxtLink to="/">
            <img
              :src="appSettings.applicationSettings.logo"
              alt=""
              style="height: 35px"
            />
          </NuxtLink>
        ...
        <ul class="menu__items d-none d-lg-flex">
          ...
          <template v-for="category in categories">
            <li
              :key="category.id"
              class="menu__item px-2"
              v-if="category.showOnMenu == 1"
            >
          ...
          <li class="menu__item px-2" v-if="!$auth.loggedIn">
            <a href="javascript:void(0)"
              >Sign In <i class="fas fa-caret-down ms-2"></i>
            </a>
        ...
        <NuxtLink
          ...
          to="/signup"
          tag="button"
          class="btn btn-outline-primary px-3 d-none d-lg-block text-decoration-none"
        >
          Join
        </NuxtLink>
    ...
    <!-- Sidebar start -->
    <div
      :class="[sidebar ? 'sidebar d-lg-none show' : 'sidebar d-lg-none']"
      ...
      <div class="sidebar__container">
        ...
        <ul class="sidebar__links">
          ...
          <template v-for="category in categories">
            <li
              :key="category.id"
              class="sidebar__item px-2"
              v-if="category.showOnMenu"
            >
              <NuxtLink :to="`/categories/${category.id}`">{{
                category.name
              }}</NuxtLink>
          ...
          <li
            ...
            v-if="!$auth.loggedIn"
            role="button"
            @click.self="toggleDropdown($event)"
          >
            <a href="javascript:void(0)" style="pointer-events: none"
              >Sign In <i class="fas fa-caret-down ms-auto"></i
            ></a>
