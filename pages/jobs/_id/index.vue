  ...
  <loading-view :loading="isLoading">
    <div v-if="job">
      ...
      <PageHeader>
        ...
        <div
          class="col-md-4 justify-content-end d-flex text-md-end"
          v-if="$auth.loggedIn"
        >
          <nuxt-link
            class="btn btn-primary px-4 me-2 text-decoration-none"
            v-if="$auth.user.type == 'seller' && job.status == 'open'"
            :to="`/bids/create?job=${job.id}`"
          >
            <span>Bid Now</span>
          </nuxt-link>
        </div>
              ...
              <div class="card border-0 rounded-0">
                <div class="card-body p-4">
                  ...
                  <div class="mt-3">
                    ...
                    <edit-attachment
                      v-if="editAttachments"
                      @close="editAttachments = false"
                    ></edit-attachment>
                  </div>
                  ...
                  <div class="mt-3">
                    ...
                    <span
                      class="mt-1 badge border border-primary text-primary rounded-pill px-3"
                    >
                      {{ job.category.name }}
                    </span>
                  </div>
                  ...
                  <div class="mt-3">
                    ...
                    <span
                      href="#"
                      class="badge border border-primary text-primary rounded-pill px-3 me-2 mt-1"
                  ...
                  <div class="mt-3">
                    <a href="#" class="text-decoration-none text-black me-3"
                      ><i class="fas fa-share-alt fa-fw text-primary"></i>
                      <span>Share this Job</span></a
                    >
                    <a href="#" class="text-decoration-none text-black"
                      ><i class="fas fa-flag fa-fw text-primary"></i>
              ...
              <div class="card border-0 rounded-0">
                <div class="card-body p-4">
                  ...
                  <ul class="ps-3 mt-3" v-if="relatedJobs.length">
                    <li
                      class="lh-lg"
                      v-for="singleJob in buyerJobs"
                      :key="singleJob.id"
                    >
                      <nuxt-link
                        class="text-decoration-none"
                        :to="'/jobs/' + singleJob.id"
                        >{{ singleJob.title }}</nuxt-link
                      >
                    </li>
                  </ul>
              ...
              <div class="card border-0 rounded-0 mt-4">
                <div class="card-body p-4">
                  ...
                  <ul class="ps-3 mt-3">
                    <li
                      class="lh-lg"
                      v-for="singleJob in similarJobs"
                      :key="singleJob.id"
                    >
                      <nuxt-link
                        class="text-decoration-none"
...
export default {
  ...
  async fetch() {
    // Get the job
    const response = await this.$axios.$get(`jobs/${this.$route.params.id}`);
