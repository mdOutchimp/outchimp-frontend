<template>
  <div v-if="pkg || seller">
    <PageHeader>
      ...
      <div class="col-md-4 text-md-end" v-if="pkg.status == 'draft'">
        <button class="btn btn-outline-primary" @click.prevent="publish">Publish</button>
      </div>
    </PageHeader>
            ...
            <div class="card rounded-0 border-0 py-3">
              <div class="card-body p-0">
                ...
                <div class="mt-4 ps-3 pe-5">
                  ...
                  <p
                    class="pt-2"
                    v-for="(highligth, index) in pkg.highlights"
                    :key="index"
                  >
                    {{ index + 1 }}. {{ highligth }}
                  </p>
              ...
              <div class="mt-3 px-3" v-if="reviews">
                ...
                <div class="mt-3" v-for="review in reviews" :key="review.id">
                  <div
                    class="d-sm-flex align-item-center justify-content-between"
                  >
                      ...
                      <div class="ms-sm-3 d-flex mt-2 mt-sm-0">
                        <div>
                          ...
                          <div>
                            ...
                            ><span class="px-1">{{ review.rating }}</span>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <div class="card-body">
                ...
                <div class="d-flex flex-column align-items-center">
                  ...
                  <div class="mt-3 text-center" v-if="seller">
                    ...
                    <p class="fw-semibold">
                      <i class="fas fa-star text-primary fa-fw"></i
                      ><span class="ps-1"> {{ seller.averageRating }} </span>
...
export default {
  ...
  methods: {
    async publish() {
      const res = await this.$axios.$post(`/packages/${this.pkg.id}/publish`);
      this.pkg = res.data;
  ...
  async fetch() {
    ...
    // Get Seller details
    const resSeller = await this.$axios.$get(`/sellers/${this.pkg.sellerId}`);
